...
// TestVmopSupportsDualStackVMServiceAPI_levels must be updated when vmopAPILevel
// gains entries (e.g. a new VM Operator API with level >= 60 for dual-stack fields).
func TestVmopSupportsDualStackVMServiceAPI_levels(t *testing.T) {
	testCases := []struct {
		name string
		ver  string
		want bool
