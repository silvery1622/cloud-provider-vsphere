...
func TestAdapter_VirtualMachineServices_DualStack_Update(t *testing.T) {
	testCases := []struct {
		name            string
		initialPolicy   *corev1.IPFamilyPolicyType
		initialFamilies []corev1.IPFamily
		updatedPolicy   *corev1.IPFamilyPolicyType
		updatedFamilies []corev1.IPFamily
		expectPolicy    *corev1.IPFamilyPolicyType
		expectFamilies  []corev1.IPFamily
	}{
		{
			name:            "nil to PreferDualStack",
			initialPolicy:   nil,
			initialFamilies: nil,
			updatedPolicy:   ptr(corev1.IPFamilyPolicyPreferDualStack),
			updatedFamilies: []corev1.IPFamily{corev1.IPv4Protocol, corev1.IPv6Protocol},
			expectPolicy:    ptr(corev1.IPFamilyPolicyPreferDualStack),
			expectFamilies:  []corev1.IPFamily{corev1.IPv4Protocol, corev1.IPv6Protocol},
		},
		{
			name:            "PreferDualStack to nil clears fields",
			initialPolicy:   ptr(corev1.IPFamilyPolicyPreferDualStack),
			initialFamilies: []corev1.IPFamily{corev1.IPv4Protocol, corev1.IPv6Protocol},
			updatedPolicy:   nil,
			updatedFamilies: nil,
			expectPolicy:    nil,
			expectFamilies:  nil,
		...
		{
			name:            "change policy only",
			initialPolicy:   ptr(corev1.IPFamilyPolicyPreferDualStack),
			initialFamilies: []corev1.IPFamily{corev1.IPv4Protocol, corev1.IPv6Protocol},
			updatedPolicy:   ptr(corev1.IPFamilyPolicyRequireDualStack),
			updatedFamilies: []corev1.IPFamily{corev1.IPv4Protocol, corev1.IPv6Protocol},
			expectPolicy:    ptr(corev1.IPFamilyPolicyRequireDualStack),
			expectFamilies:  []corev1.IPFamily{corev1.IPv4Protocol, corev1.IPv6Protocol},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			initial := &vmoptypes.VirtualMachineServiceInfo{
				Name:      "dual-update-vms",
				Namespace: testNS,
				Spec: vmoptypes.VirtualMachineServiceSpec{
					Type:           vmoptypes.VirtualMachineServiceTypeLoadBalancer,
					IPFamilies:     tc.initialFamilies,
					IPFamilyPolicy: tc.initialPolicy,
				},
			}
			_, err := adapter.VirtualMachineServices().Create(ctx, initial)
			assert.NoError(t, err)

			// Update with new dual-stack fields.
			updateInfo := &vmoptypes.VirtualMachineServiceInfo{
				Spec: vmoptypes.VirtualMachineServiceSpec{
					IPFamilies:     tc.updatedFamilies,
					IPFamilyPolicy: tc.updatedPolicy,
				},
			}
			_, err = adapter.VirtualMachineServices().Update(ctx, testNS, "dual-update-vms", updateInfo)
			assert.NoError(t, err)

			// Verify via Get that the fields were persisted.
			got, err := adapter.VirtualMachineServices().Get(ctx, testNS, "dual-update-vms")
			assert.NoError(t, err)
			assert.Equal(t, tc.expectFamilies, got.Spec.IPFamilies)
			if tc.expectPolicy == nil {
				assert.Nil(t, got.Spec.IPFamilyPolicy)
			} else {
				assert.NotNil(t, got.Spec.IPFamilyPolicy)
				assert.Equal(t, *tc.expectPolicy, *got.Spec.IPFamilyPolicy)
			}
		})
	}
}
...
// TestAdapter_VirtualMachineServices_Get_DualStack_TypeMismatch verifies that
// readDualStackFromUnstructured propagates a type-mismatch error to the caller
// rather than logging a warning and silently clearing the field. The malformed
// object is injected directly into the fake client so it bypasses the API
// server's schema validation.
func TestAdapter_VirtualMachineServices_Get_DualStack_TypeMismatch(t *testing.T) {
	adapter, fc := fakev6.NewAdapter()

	// spec.ipFamilies is an integer instead of a string array — an invalid type
	// that should cause readDualStackFromUnstructured to return an error.
	obj := &unstructured.Unstructured{
		Object: map[string]interface{}{
			"apiVersion": clientv6.VirtualMachineServiceGVR.Group + "/" + clientv6.VirtualMachineServiceGVR.Version,
			...
			"spec": map[string]interface{}{
				"type":       "LoadBalancer",
				"ipFamilies": float64(42), // wrong type: scalar instead of []string; float64 is the JSON-native number type
			},
		},
	}
	_, err := fc.Resource(clientv6.VirtualMachineServiceGVR).Namespace(testNS).Create(
		context.Background(), obj, metav1.CreateOptions{})
	assert.NoError(t, err)

	got, err := adapter.VirtualMachineServices().Get(context.Background(), testNS, "bad-type-vms")
	assert.Error(t, err)
	assert.Nil(t, got)
}
