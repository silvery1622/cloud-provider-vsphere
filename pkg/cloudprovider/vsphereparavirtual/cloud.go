...
var (
	...
	//svcAnnoPropEnabled if set to true, will propagate the service annotation to resource in supervisor cluster.
...
func init() {
	...
	flag.BoolVar(&svcAnnoPropEnabled, "service-annotation-propagation", false, "If true, will propagate the service annotation to resource in supervisor cluster.")
}
...
func (cp *VSphereParavirtual) Initialize(clientBuilder cloudprovider.ControllerClientBuilder, stop <-chan struct{}) {
	...
	lb, err := NewLoadBalancer(clusterNS, kcfg, cp.ownerReference, svcAnnoPropEnabled)
	if err != nil {
