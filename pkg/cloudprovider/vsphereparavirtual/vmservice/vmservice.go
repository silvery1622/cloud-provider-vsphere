...
const (
	...
	// virtualMachineServiceTypeLoadBalancer is the LoadBalancer type from the hub types package.
	virtualMachineServiceTypeLoadBalancer = vmoptypes.VirtualMachineServiceTypeLoadBalancer
)
...
func (s *vmService) Update(ctx context.Context, service *v1.Service, clusterName string, existing *vmoptypes.VirtualMachineServiceInfo) (*vmoptypes.VirtualMachineServiceInfo, error) {
	...
	if existing.Spec.LoadBalancerSourceRanges == nil {
		existing.Spec.LoadBalancerSourceRanges = []string{}
	}
	if service.Spec.LoadBalancerSourceRanges == nil {
		service.Spec.LoadBalancerSourceRanges = []string{}
	}

	annotations := getVMServiceAnnotations(existing, service, s.serviceAnnotationPropagationEnabled)
	...
	if !reflect.DeepEqual(existing.Spec.LoadBalancerSourceRanges, service.Spec.LoadBalancerSourceRanges) {
		needsUpdate = true
	}
...
func findPorts(service *v1.Service) ([]vmoptypes.VirtualMachineServicePort, error) {
	var ports []vmoptypes.VirtualMachineServicePort
...
func (s *vmService) lbServiceToVMServiceInfo(service *v1.Service, clusterName string) (*vmoptypes.VirtualMachineServiceInfo, error) {
	...
	info := &vmoptypes.VirtualMachineServiceInfo{
		...
		Spec: vmoptypes.VirtualMachineServiceSpec{
			Type:                     virtualMachineServiceTypeLoadBalancer,
			Ports:                    ports,
			Selector:                 selector,
			LoadBalancerIP:           service.Spec.LoadBalancerIP,
			LoadBalancerSourceRanges: service.Spec.LoadBalancerSourceRanges,
	...
	if annotations := getVMServiceAnnotations(info, service, s.serviceAnnotationPropagationEnabled); len(annotations) != 0 {
...
func getVMServiceAnnotations(vms *vmoptypes.VirtualMachineServiceInfo, service *v1.Service, serviceAnnotationPropagationEnabled bool) map[string]string {
